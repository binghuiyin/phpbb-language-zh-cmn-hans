...
$lang = array_merge($lang, array(
	...
	'SYSTEM_TIMEZONE_EXPLAIN'         => '时区用来为未登录的用户（游客，爬虫）显示时区。新用户可以在注册过程中选择时区或在登录后进入用户控制面板进行修改。',
...
$lang = array_merge($lang, array(
	...
	'AVATAR_GALLERY_PATH_EXPLAIN'	=> '在您的 phpBB 论坛根目录下的预先设定的系统头像路径，例如 <samp>images/avatars/gallery</samp>',
...
$lang = array_merge($lang, array(
	...
	'NEW_MEMBER_GROUP_DEFAULT_EXPLAIN'	=> '如果设置为是, 并且设定了【新会员发文限制】，新会员不仅会加入到 <em>新注册用户</em> 组，这个用户组也会成为他的默认组。这将方便管理员为该组指定一个默认等级和组头像。',
...
$lang = array_merge($lang, array(
	...
	'ACP_VC_EXT_GET_MORE'					=> '对于附加的(可能更好)反垃圾邮件插件，访问 <a href="https://www.phpbb.com/go/anti-spam-ext"><strong>phpBB.com 扩展数据库</strong></a>。更多防止论坛垃圾邮件的信息，访问 <a href="https://www.phpbb.com/go/anti-spam"><strong>phpBB.com知识库</strong></a> 。',
...
$lang = array_merge($lang, array(
	...
	'COOKIE_SECURE_EXPLAIN'		=> '如果您的服务器使用 SSL 协议，则启用这个选项，否则请禁用. 如果没有使用 SSL 而启用这个选项，将会在页面转向时出现错误。',
...
// Contact Settings
$lang = array_merge($lang, array(
	'ACP_CONTACT_SETTINGS_EXPLAIN'		=> '这里您可以启用或禁用联系页也可以添加文本显示在页面。',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_CDN_EXPLAIN'				=> '如果此设置启用，一些文件将被第三方服务器提供，而不是您的服务器。这降低了网络带宽需求，但可能对于一些论坛管理员出现隐私问题。默认 phpBB 安装时这个包括加载 “jQuery” 和字体 “Open Sans” 从 Google 的内容分发网络。',
	'ALLOW_LIVE_SEARCHES'			=> '允许实时搜索',
	'ALLOW_LIVE_SEARCHES_EXPLAIN'	=> '如果此设置启用，用户输入时可看到关键词提示。',
	'CUSTOM_PROFILE_FIELDS'			=> '自定义用户资料',
	'LIMIT_LOAD'					=> '系统负载限制',
	'LIMIT_LOAD_EXPLAIN'			=> '如果系统一分钟内的平均负载超过这个值论坛将自动关闭。值 1.0 等于单颗处理器的100%使用率，这只工作于基于UNIX/Linux的系统。',
	'LIMIT_SESSIONS'				=> '会话数限制',
	'LIMIT_SESSIONS_EXPLAIN'		=> '如果一分钟内的会话数超过这个值论坛将自动关闭。设置为 0 将不作限制。',
	'LOAD_CPF_MEMBERLIST'			=> '允许风格在会员列表中显示自定义资料',
	'LOAD_CPF_PM'					=> '在私人短信里显示自定义资料',
...
// Auth settings
$lang = array_merge($lang, array(
	'ACP_AUTH_SETTINGS_EXPLAIN'	=> 'phpBB 支持认证插件和模块。这允许您决定如何验证用户是否登录。默认的三个插件是 DB，LDAP 和 Apache。并不是所有方式都需要额外信息，所以您只需要填写选中的方式需要的信息即可。',

	'AUTH_METHOD'				=> '选择认证方式',

	'AUTH_PROVIDER_OAUTH_ERROR_ELEMENT_MISSING'	=> '每个启用的 OAuth 服务的密钥和密码都要提供。只有提供一个被提供给 OAuth 服务商。',
	...
	'LDAP_SERVER_EXPLAIN'			=> '如果使用 LDAP，这是 LDAP 服务器的域名或 IP 地址。并且您也可以指定一个URL例如 ldap://hostname:port/',
	'LDAP_UID'						=> 'LDAP <var>uid</var>',
	'LDAP_UID_EXPLAIN'				=> '这是用于查找给定登录身份的关键字，例如 <var>uid</var>, <var>sn</var>。',
	'LDAP_USER'						=> 'LDAP 用户 <var>dn</var>',
	'LDAP_USER_EXPLAIN'				=> '如果绑定为匿名，此处请留空。如果填入，phpBB 会使用指定的唯一用户名在登录中寻找正确的用户，例如： <samp>uid=Username,ou=MyUnit,o=MyCompany,c=US</samp>。这对于 Active Directory Server 是必需的。',
...
$lang = array_merge($lang, array(
	...
	'MOD_REWRITE_ENABLE_EXPLAIN' => '启用后包含 ’app.php’ 的路径将重写为移除文件名(例如app.php/foo 变成 /foo)。<strong>必须有Apache服务器的 mod_rewrite 组件才能正常工作；如果此选项启用但没有 mod_rewrite，路径将损坏。</strong>',
	'MOD_REWRITE_DISABLED'		=> '<strong>mod_rewrite</strong>组件在您的Apache服务器上禁用，启用该组件或联系主机提供商如果您想启用此功能。',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_PASSWORD_RESET_EXPLAIN'	=> '确定是否能使用"我忘了密码"链接在登录页恢复账户。如果使用外部验证机制您可能想禁用此功能。',
	...
	'FORM_SID_GUESTS_EXPLAIN'		=> '启用后, 游客所有的表单将会是对话唯一的，这个功能在某些 ISP 中可能无法正常使用。',
	...
	'IP_VALID'						=> '对话 IP 验证',
	'IP_VALID_EXPLAIN'				=> '决定用户的 IP 如何用于会话验证; <samp>所有</samp> 表示完整地址。<samp>A.B.C</samp> 表示开头的 x.x.x, <samp>A.B</samp> 表示开头的 x.x, <samp>None</samp> 取消验证.',
...
$lang = array_merge($lang, array(
	...
	'ENABLE_EMAIL_EXPLAIN'			=> '如果禁用，论坛将不会发送任何 email。<em>注意如果当前设定为用户激活或者管理员激活，则需要启用此项功能，若停用此项功能将使会员无法激活。</em>',
	'SMTP_AUTH_METHOD'				=> 'SMTP 验证方式',
	'SMTP_AUTH_METHOD_EXPLAIN'		=> '只有在设置过用户名/密码的情况下才使用，如果您不能确定使用何种方式，请询问你的 Email 服务商。',
	...
	'SMTP_SERVER_EXPLAIN'			=> '请提供服务器所使用的端口。如果有用到 SSL ，应该是这样的地址： "ssl://your.mailserver.com"',
	'SMTP_SETTINGS'					=> 'SMTP 设定',
	'SMTP_USERNAME'					=> 'SMTP 用户名',
	'SMTP_USERNAME_EXPLAIN'			=> '只有当您的 SMTP 服务器需要时才要输入。',
	'USE_SMTP'						=> '使用 SMTP 服务器发送 email',
	'USE_SMTP_EXPLAIN'				=> '如果您想通过其他服务器而不是本地 mail 函数发送 email，就选择“是”。',
));
