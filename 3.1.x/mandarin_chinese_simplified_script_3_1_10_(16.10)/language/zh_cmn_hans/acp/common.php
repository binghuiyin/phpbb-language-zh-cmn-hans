...
// PHP info
$lang = array_merge($lang, array(
	'ACP_PHP_INFO_EXPLAIN'	=> '这个页面列出了这个服务器上所安装的 PHP 的信息。这包括装载的模块，可用的变量和默认设定，在诊断问题时这些信息可能会有用。请注意一些主机服务商可能会因为安全因素禁止显示某些信息。您尽可能不要泄露这些信息，除非技术支持中的 <a href="https://www.phpbb.com/about/">官方开发成员</a> 需要这些信息。',
...
$lang = array_merge($lang, array(
	...
	'ACP_CRITICAL_LOGS_EXPLAIN'	=> '这里列出了论坛自身的操作。这些日志为您解决特定问题提供有用信息，例如无法投递的 email。您可以按用户名、日期、IP地址或操作排序。如果您有足够的权限，您也可以清除部分或全部日志记录。',
...
$lang = array_merge($lang, array(
	...
	'PHP_VERSION_OLD'	=> '当前服务器上的 PHP 版本太旧，新的 phpBB 将不再支持这个版本的 PHP。%s详情%s',
...
$lang = array_merge($lang, array(
	...
	'INACTIVE_USERS_EXPLAIN_INDEX'	=> '这是最近10个已注册但未激活的用户列表。您可以激活、删除或提醒（通过 email）这些用户。账户未激活可能是因为在用户注册设置里已启用账号激活（邮箱验证或者管理员激活）但这些用户还没被激活，也可能因为这些账号被冻结。点击下面的链接可得到一份完整的列表。',
